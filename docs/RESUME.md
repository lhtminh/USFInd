...
- **Item count seeded** for the benchmark window

From the `/stats` page (Next.js, fed by `/api/stats`) after some live use:
...
> + NVIDIA Nemotron 3 Nano Omni 30B reasoning model re-ranking with structured
> JSON output, served via OpenRouter) achieving Stage 1 p95 of **{X} ms** over
> **{N}** items and end-to-end p95 of **{Y} s** on warm cache, with per-match
...
> rerank hit rate of **{Z} %** during benchmark replay. Defaults to a
> free-tier OpenRouter model so per-query LLM cost is effectively zero;
> swapping to a paid model is a one-env-var change.

### 4. LLM product features

> Shipped three LLM-powered features behind a structured-output schema:
...
> all wrapped with retry, JSON-extraction tolerance for reasoning tokens,
> and full token+cost telemetry into a Postgres `llm_usage` table for
> per-endpoint cost tracking. Provider-agnostic: the same surface
> (`call_flash`, `call_pro`, cached variants) sits over any OpenAI-compatible
> backend.
...
3. How does the system behave when the LLM is rate-limited or returns invalid JSON?
...
### LLM features
1. How do you guarantee JSON output from a reasoning LLM, and what happens when it deviates?
2. Why was the search parser split out from the recall step?
3. How is per-endpoint cost attribution implemented, and what changes when you swap providers?
